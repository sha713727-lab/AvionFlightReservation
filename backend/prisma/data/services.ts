export const SERVICE_SEED = [
  {
    slug: 'flights',
    title: 'Flight Booking',
    tagline: 'Clear fare options and flexible routes to destinations worldwide.',
    description:
      'Our travel specialists help you book flights by phone across Canada, the USA, Europe, Mexico, and worldwide using available routes and published fares.',
    features: [
      'Compare published fares across major airlines',
      'Multi-city and complex routing handled for you',
      'Guidance on cabin options and fare trade-offs',
      'Phone assistance before, during, and after your flight within staffed hours',
    ],
    iconKey: 'plane',
    imageKey: 'flight-booking',
    imageAlt: 'Travel specialist booking airline flights by phone',
    sortOrder: 1,
  },
  {
    slug: 'hotels',
    title: 'Hotel Booking',
    tagline: 'Handpicked stays from boutique hotels to luxury resorts.',
    description:
      'We arrange hotel reservations that match your style and budget, from city-center business hotels to secluded beachfront resorts.',
    features: [
      'Personally vetted hotel recommendations',
      'Rate types and cancellation windows explained',
      'Resort and facility fees flagged when shown',
      'Flexible cancellation options when available',
    ],
    iconKey: 'hotel',
    imageKey: 'hotel-booking',
    imageAlt: 'Hotel room ready for phone hotel reservation booking',
    sortOrder: 2,
  },
  {
    slug: 'rewards',
    title: 'Points Redemption Help',
    tagline: 'Maximize your points and miles for premium travel experiences.',
    description:
      'Turn airline miles and credit card rewards into travel. Our specialists help compare redemption options and book the trip that fits your points value.',
    features: [
      'Points & miles valuation and strategy',
      'Award cabin options explained clearly',
      'Transfer partner timing guidance',
      'End-to-end redemption booking help',
    ],
    iconKey: 'gift',
    imageKey: 'reward-travel',
    imageAlt: 'Airplane cabin for points and miles redemption travel',
    sortOrder: 3,
  },
  {
    slug: 'changes',
    title: 'Flight Changes',
    tagline: 'Quick rebooking and itinerary adjustments with expert support.',
    description:
      'Plans change — we handle flight change assistance, airline policy review, and rebooking support with minimal hassle.',
    features: [
      'Same-day change assistance when available',
      'Airline policy navigation on your behalf',
      'Alternative routing when flights cancel',
      'Credit and voucher management guidance',
    ],
    iconKey: 'exchange',
    imageKey: 'flight-change',
    imageAlt: 'Airport departure board during flight change rebooking',
    sortOrder: 4,
  },
  {
    slug: 'cancellation',
    title: 'Cancellation',
    tagline: 'Hassle-free cancellations with guidance on refunds and credits.',
    description:
      'Need flight cancellation support? We manage refunds, credits, and supplier communication so every eligible option is reviewed.',
    features: [
      'Full cancellation handling support',
      'Refund and credit outcome review',
      'Travel insurance claim pointers',
      'Future travel credit tracking notes',
    ],
    iconKey: 'ban',
    imageKey: 'cancellation',
    imageAlt: 'Boarding pass for flight cancellation and refund help',
    sortOrder: 5,
  },
  {
    slug: 'seats',
    title: 'Seat Selection',
    tagline: 'Secure preferred seats for comfort on every journey.',
    description:
      'From extra legroom to window seats and family seating together, we manage airline seat selection for a smoother journey.',
    features: [
      'Preferred seat assignments when maps allow',
      'Extra legroom and exit row booking help',
      'Family and group seating together',
      'Last-minute seat change assistance',
    ],
    iconKey: 'chair',
    imageKey: 'seat-selection',
    imageAlt: 'Airplane cabin seats for airline seat selection help',
    sortOrder: 6,
  },
  {
    slug: 'baggage',
    title: 'Baggage',
    tagline: 'Extra baggage allowances and priority handling arranged for you.',
    description:
      'Avoid surprise fees at the airport. We arrange baggage booking, excess baggage, sports equipment transport, and priority handling in advance.',
    features: [
      'Pre-paid excess baggage booking help',
      'Sports and oversized equipment guidance',
      'Priority baggage handling when offered',
      'Lost luggage claim process pointers',
    ],
    iconKey: 'suitcase',
    imageKey: 'baggage',
    imageAlt: 'Checked luggage for airline baggage fee assistance',
    sortOrder: 7,
  },
  {
    slug: 'planning',
    title: 'Trip Planning',
    tagline: 'Custom itineraries crafted by experienced travel specialists.',
    description:
      'Tell us your travel goals and we build a custom trip planning itinerary with flights, hotels, transfers, activities, and local recommendations.',
    features: [
      'Fully customized day-by-day itineraries',
      'Local experience recommendations',
      'Ground transfers and logistics planning',
      'Real-time itinerary adjustment support',
    ],
    iconKey: 'route',
    imageKey: 'trip-planning',
    imageAlt: 'World map and passport for custom trip planning',
    sortOrder: 8,
  },
  {
    slug: 'packages',
    title: 'Trip Planning Assistance',
    tagline: 'Help comparing flight and hotel legs for a coordinated trip plan.',
    description:
      'Independent trip-planning assistance for travelers who want help comparing flight and hotel options together. This is planning support with a separately quoted assistance fee—not a guaranteed all-inclusive package inventory or special fare authority.',
    features: [
      'Coordinated flight and hotel option comparison',
      'Written summary of options within agreed scope',
      'Clear separation of our fee from supplier charges',
      'Customer-controlled booking steps through official channels',
    ],
    iconKey: 'beach',
    imageKey: 'vacation-package',
    imageAlt: 'Trip planning assistance for flights and hotels',
    sortOrder: 9,
  },
] as const
