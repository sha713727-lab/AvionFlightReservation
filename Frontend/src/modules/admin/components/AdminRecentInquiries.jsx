...
import { ADMIN_INQUIRIES_PATH } from '@/constants/routes'
...
export default function AdminRecentInquiries({ items }) {
  return (
    <section className="rounded-2xl border border-border bg-section p-5 shadow-sm shadow-primary/5">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-heading text-lg font-semibold tracking-tight text-primary">
          {ADMIN_COPY.recentInquiriesTitle}
        </h2>
        <Link
        ...
        >
          {ADMIN_COPY.viewInquiriesCta}
        </Link>
      </div>

      {items.length === 0 ? (
        <p className="mt-6 text-sm text-text-secondary">{ADMIN_COPY.emptyInquiries}</p>
      ) : (
        <ul className="mt-5 space-y-3">
          {items.map((item) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-text-secondary">{item.subject}</p>
              <p className="mt-1 text-xs text-text-muted">
                {ADMIN_COPY.inquiriesReferenceLabel}: {item.referenceCode}
                {' · '}
